...
class my_dataset(Dataset):
    def __init__(self,data_iter,d):
        self.data = data_iter
        self.d = d
        self.vocab = {'GGG':0, 'GGA':1, 'GGC':2, 'GGU':3,'GAG':4, 'GAA':5, 'GAC':6, 'GAU':7,
                      'GCG': 8, 'GCA': 9, 'GCC': 10, 'GCU': 11, 'GUG': 12, 'GUA': 13, 'GUC': 14, 'GUU': 15,
                      'AGG': 16, 'AGA': 17, 'AGC': 18, 'AGU': 19, 'AAG': 20, 'AAA': 21, 'AAC': 22, 'AAU': 23,
                      'ACG': 24, 'ACA': 25, 'ACC': 26, 'ACU': 27, 'AUG': 28, 'AUA': 29, 'AUC': 30, 'AUU': 31,
                      'CGG': 32, 'CGA': 33, 'CGC': 34, 'CGU': 35, 'CAG': 36, 'CAA': 37, 'CAC': 38, 'CAU': 39,
                      'CCG': 40, 'CCA': 41, 'CCC': 42, 'CCU': 43, 'CUG': 44, 'CUA': 45, 'CUC': 46, 'CUU': 47,
                      'UGG': 48, 'UGA': 49, 'UGC': 50, 'UGU': 51, 'UAG': 52, 'UAA': 53, 'UAC': 54, 'UAU': 55,
                      'UCG': 56, 'UCA': 57, 'UCC': 58, 'UCU': 59, 'UUG': 60, 'UUA': 61, 'UUC': 62, 'UUU': 63}
    def __len__(self):
...
class TextCNN(nn.Module):
    def __init__(self,embedding_size,window_size,n_filters,dropout=0.4):
        ...
        self.embed = nn.Embedding(64,embedding_size)
...
if __name__ == '__main__':
    ...
    np.savetxt('pr3_p',pr_data[0],fmt='%f')
    np.savetxt('pr3_r',pr_data[1],fmt='%f')
    print(performance)
    print(roc_data)
    print('-----------------------------')
    print(pr_data)
